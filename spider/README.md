...
4. **图片下载**：自动下载图片到 `crawled_images` 文件夹，支持智能重试机制：
   - 连接超时：10秒
   - 读取超时：40秒
   - 最多重试3次
   - 针对服务器错误（502/503/504/429）使用更长延迟（5秒、10秒、15秒）
   - 普通错误使用递增延迟（2秒、4秒、6秒）
   - 图片下载之间自动添加1.5秒延迟，避免对服务器造成过大压力
5. **标签提取**：自动从页面内容中提取标签并存储
6. **智能过滤**：自动过滤非文化相关的图片（如logo、图标、横幅、二维码、维基百科特殊URL等）
7. **默认图片处理**：如果文字资源没有图片，自动插入default.jpg记录
8. **时间限制**：每个爬虫运行30分钟，自动停止（不再限制资源数量）
...
6. **时间限制**：每个爬虫运行30分钟后自动停止，不再限制资源数量
...
1. **502 Bad Gateway / 503 Service Unavailable**：
   - 原因：目标服务器暂时不可用或过载
   - 处理：爬虫会自动重试，使用更长的延迟时间（5秒、10秒、15秒）
   - 建议：如果持续出现，可以稍后重试

2. **超时错误**：
   - 原因：网络连接慢或服务器响应慢
   - 处理：爬虫已设置连接超时10秒、读取超时40秒，会自动重试
   - 建议：检查网络连接质量

3. **其他网络错误**：
   - 原因：网络不稳定或目标网站限制
   - 处理：爬虫会自动重试，使用递增延迟（2秒、4秒、6秒）
   - 建议：确保网络连接稳定

4. **SSL错误**：
   - 原因：某些网站的SSL证书问题
   - 处理：爬虫会自动禁用SSL验证作为备选方案
   - 注意：仅在必要时使用，可能存在安全风险

优化措施：
- 图片下载之间自动添加1.5秒延迟，避免请求过快
- 针对不同类型的错误使用不同的重试策略
- 自动跳过无法下载的图片，继续处理其他图片
