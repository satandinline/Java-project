...
- **角色管理**：管理员/普通用户
- **默认账号**：
  - 管理员：账号 `123456789`，密码 `123456`，二级问题：我的身份是？，答案：管理员
  - 测试用户：账号 `987654321`，密码 `123456`，二级问题：我的身份是？，答案：测试用户
  - （首次登录后请立即修改密码）
...
### 3. 搜索功能
- **全文检索**：支持关键词搜索，使用AI语义分析优化检索式
  - 支持搜索`cultural_entities`和`AIGC_cultural_entities`表
  - 自动关联`crawled_images`表获取图片信息
  - 检索结果正确显示图片URL，支持多种图片路径格式
  - 如果资源没有图片，自动使用默认图片
- **AI检索**：使用AI模型理解用户需求，结合向量检索提供智能回答
  - 支持向量检索和数据库检索
  - 返回AI分析结果和建议
  - 检索结果正确显示图片URL
...
- **文字AIGC（Tongyi模型）**：基于RAG的智能问答和文化资源生成
  ...
  - 用户发送消息后立即保存并显示，不等待AI生成完成（使用UPDATE机制避免重复）
  - 检索资源持久化显示，支持折叠/展开，点击资源名称可查看详情
  - 检索资源ID正确保存到qa_messages表的retrieval_id字段
  - AIGC文字资源在首页显示时使用 `AIGC_graph/default.jpg` 作为图片路径
  - 如果没有用户上传图片，使用 `AIGC_graph/default.jpg` 作为默认图片
- **图片AIGC（Huoshan模型）**：图像生成和漫画/连环画创作
  - 支持上传图片并理解图片内容
  - 无文字提示时自动生成故事并生成连环画
  - 生成的图片以假乱真
  - 用户发送消息后立即保存并显示，不等待AI生成完成（使用UPDATE机制避免重复）
  - 用户上传的AIGC图片统一存入 `AIGC_graph_from_users` 文件夹
  - 如果没有上传图片，使用 `AIGC_graph/default.jpg` 作为默认图片
- 支持多轮对话和历史会话管理（会话和消息存储在 `qa_sessions` 和 `qa_messages` 表）
- 文字和图片AIGC都支持图片上传
- 自动保存AIGC生成的内容到数据库（文字资源保存检索资源ID列表到retrieval_id字段）
- 支持会话删除（单个、批量、全部）
- 检索功能优化：支持更多字段检索，包括用户上传资源表

### 5. 资源上传
- **文本资源**：
  - 支持直接输入文本内容（无需上传文件）
  - 支持上传Word文档（.doc, .docx）或PDF（.pdf）文件
  - 自动解析文件内容用于AI标注
- **图像资源**：
  - 支持上传JPG（.jpg）或PNG（.png）图片
- **上传时标注**：
  - 用户可在上传时进行标注（格式与AI标注、人工标注一致）
  - 支持完整的标注字段（实体名称、类型、描述、来源等）
- **自动处理与补偿**：
  - 自动查重和审核流程，标注任务自动创建
  - 检测到用户上传资源后立即触发AI标注；后台每5分钟轮询一次“待标注”任务，防止遗漏
  - 人工标注审核通过后自动迁移：资源写入 `cultural_resources`，实体写入 `cultural_entities`，图片写入 `crawled_images`；无图时自动使用 `uploads/default.jpg`

### 6. 标注任务
- 支持三种资源来源：爬虫抓取、用户上传、AIGC生成
- 任务状态：待标注、AI标注中、AI标注完成、已完成
- 权限控制：普通用户只能看到自己上传的资源任务，管理员可看全部
- 查看标注：只读模式，不可编辑
- 编辑标注：可编辑模式，支持修改所有标注字段
- 人工标注审核通过后自动迁移：资源写入 `cultural_resources`，实体写入 `cultural_entities`，图片写入 `crawled_images`；无图时自动使用 `uploads/default.jpg`
...
- Python AIGC服务：http://localhost:7200（如需要AI功能）
...
### 主要手册（同时提供.md和.txt版本）
- [技术实现手册](技术实现手册.md) / [技术实现手册.txt](技术实现手册.txt) - 系统架构、技术栈、API文档、代码规范
- [用户使用手册](用户使用手册.md) / [用户使用手册.txt](用户使用手册.txt) - 用户操作指南、功能说明、常见问题
...
- **`AIGC_graph_from_users/`**：存储用户上传的AIGC图片（用户上传用于AIGC的图片）
